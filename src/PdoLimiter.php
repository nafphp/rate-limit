...
final class PdoLimiter
{
    ...
    {
        $this->pdo->exec('CREATE TABLE IF NOT EXISTS naf_rate_limits (bucket VARCHAR(64) PRIMARY KEY,window_start BIGINT NOT NULL,hits BIGINT NOT NULL,expires_at BIGINT NOT NULL)');
    }
    ...
    public function consume(string $key, int $limit, int $windowSeconds, ?int $now = null): array
    {
        ...
        try {
            ...
            $this->pdo->commit();
            return ['allowed' => $hits <= $limit,'remaining' => max(0, $limit - $hits),'retry_after' => max(1, $expires - $now)];
        } catch (\Throwable $e) {
            if ($this->pdo->inTransaction()) {
                $this->pdo->rollBack();
            }throw $e;
